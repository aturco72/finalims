  ...
  <ion-page>
    <ion-header :translucent="true">
      <ion-toolbar color="secondary">
        ...
        <ion-title>Cedolino Irrigazione</ion-title>
    ...
    <ion-header>
      ...
      <ion-toolbar v-if="step == 3">
        <ion-segment :value="tuttipozzi">
          <ion-segment-button value="" v-on:click="tuttipozzi = ''">
            Tutti i pozzi
          </ion-segment-button>
          <ion-segment-button
            :value="sedesel.cd_sede"
            v-on:click="tuttipozzi = sedesel.cd_sede"
          >
            {{ sedesel.cd_sede }}
          </ion-segment-button>
        </ion-segment>
    ...
    <ion-content fullscreen>
      <div v-if="step == 0">
        <ion-card
          v-for="(sede, i) in sedi"
          :key="i"
          v-on:click="
            sedesel = sede;
            tuttipozzi = sede.cd_sede;
            getstato();
        ...
        >
          <ion-card-header>
            <ion-card-subtitle> Sede: </ion-card-subtitle>
            <ion-card-title>{{ sede.cd_sede }}</ion-card-title>
          </ion-card-header>
        </ion-card>
      </div>
      <div v-if="step == 1">
              ...
              <div v-for="(itm, t) in sezione.itm" :key="t">
                <ion-item lines="full" :color="itm.seleziona == 1 ? '' : ''">
                  ...
                  <ion-label
                    ...
                    "
                  >
                    <b :style="{ color: !itm.dt_sgombro ? 'green' : 'red' }">{{
                      itm.Denominazione
                    }}</b>

                    <span style="color: blue; float: right">
                      {{ numToString(parseFloat(selareatun(itm)), 4, 4) }}</span
                    >
                    <br />
      ...
      <div v-if="step == 3">
        <ion-card
          v-for="(pozzo, i) in pozzi.filter(
            (value) => tuttipozzi == '' || value.cd_sede == sedesel.cd_sede
          )"
          :key="i"
          v-on:click="
            step = 4;
            pozzosel = pozzo;
            cedolino.IR_Pozzo = pozzo.cd_anagrafica;
            cedolino.IR_Liniziale = pozzo.lastconta;
            getoperatori();
          "
          color="success"
        >
          <ion-card-header>
            <ion-card-subtitle>
              Matr.: {{ pozzo.Matricola }}
              <span style="float: right">
                Contatore: {{ pozzo.lastconta }}
              </span>
            </ion-card-subtitle>
            <!--
            <ion-card-title>{{ pozzo.cd_sede }}</ion-card-title>
            -->
          </ion-card-header>
          <ion-card-content>
            <b style="color: yellow">
              {{ pozzo.cd_sede }} - {{ pozzo.Descrizione }}
            </b></ion-card-content
          >
        </ion-card>
      </div>
...
export default defineComponent({
  ...
  mounted() {
    ...
    self.tuttipozzi = self.sedesel.cd_sede;
  },
  methods: {
    async annulla() {
